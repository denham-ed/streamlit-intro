...
def calculator_body():
    ...
    with col2:
        num2 = st.number_input(label='Enter the second integer', step=1)
    
    with col3:
        operator = st.selectbox(label='Select an operator', options = ['Add','Subtract','Multiply','Divide'])

    if st.button('Click here to calculator'):
        if num2 == 0 and operator == 'Divide':
            st.error("Stop trying to divide by 0!")
        else:
